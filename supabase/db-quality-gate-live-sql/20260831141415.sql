...
CREATE TABLE public.device_quota_regulatory_source_positions (
  ...
  FOREIGN KEY (catalog_version_id, parent_source_identifier)
    REFERENCES public.device_quota_regulatory_source_positions (catalog_version_id, source_identifier)
);
...
CREATE OR REPLACE FUNCTION device_quota_internal.catalog_is_complete(p_catalog_version_id UUID)
RETURNS BOOLEAN LANGUAGE SQL STABLE AS $$
  SELECT EXISTS (
    SELECT 1 FROM public.device_quota_regulatory_catalog_versions AS v
    WHERE v.id = p_catalog_version_id
      AND (SELECT count(*) FROM public.device_quota_regulatory_source_positions p WHERE p.catalog_version_id = v.id) = v.expected_structural_rows
      AND (SELECT count(*) FROM public.device_quota_regulatory_sections s WHERE s.catalog_version_id = v.id) = v.expected_section_rows
      AND (SELECT count(*) FROM public.device_quota_regulatory_items i WHERE i.catalog_version_id = v.id) = v.expected_item_rows
      AND (SELECT count(*) FROM public.device_quota_regulatory_items i WHERE i.catalog_version_id = v.id AND i.section_id IS NOT NULL) = v.expected_child_item_rows
      AND (SELECT count(*) FROM public.device_quota_regulatory_items i WHERE i.catalog_version_id = v.id AND i.section_id IS NULL) = v.expected_top_level_item_rows
      AND (SELECT count(*) FROM public.device_quota_regulatory_rules r JOIN public.device_quota_regulatory_items i ON i.id = r.item_id WHERE i.catalog_version_id = v.id) = v.expected_rule_lines
      AND (SELECT count(*) FROM public.device_quota_regulatory_references r WHERE r.catalog_version_id = v.id AND r.reference_type = 'footnote') = v.expected_footnotes
      AND (SELECT count(DISTINCT p.item_id) FROM public.device_quota_regulatory_source_positions p WHERE p.catalog_version_id = v.id AND p.item_id IS NOT NULL) = v.expected_item_rows
      AND (SELECT count(DISTINCT p.item_id) FROM public.device_quota_regulatory_source_pages sp JOIN public.device_quota_regulatory_source_positions p ON p.id = sp.source_position_id WHERE p.catalog_version_id = v.id AND p.item_id IS NOT NULL) = v.expected_items_with_source_pages
      AND (SELECT count(*) FROM public.device_quota_regulatory_references r JOIN public.device_quota_regulatory_source_positions p ON p.id = r.source_position_id WHERE r.catalog_version_id = v.id AND r.reference_type = 'source' AND p.item_id IS NOT NULL) = v.expected_items_with_source_references
      AND (SELECT count(*) FROM public.device_quota_regulatory_items i WHERE i.catalog_version_id = v.id AND cardinality(i.quota_lines) > 1) = v.expected_multiline_items
      AND (SELECT count(*) FROM public.device_quota_regulatory_source_positions p WHERE p.catalog_version_id = v.id AND NOT EXISTS (SELECT 1 FROM public.device_quota_regulatory_source_pages sp WHERE sp.source_position_id = p.id)) = 0
      AND (SELECT count(*) FROM public.device_quota_regulatory_source_positions p WHERE p.catalog_version_id = v.id AND NOT EXISTS (SELECT 1 FROM public.device_quota_regulatory_references r WHERE r.source_position_id = p.id AND r.reference_type = 'source')) = 0
  );
$$;
...
BEGIN
  IF NEW.import_status = 'ready' AND NOT device_quota_internal.catalog_is_complete(NEW.id) THEN
    RAISE EXCEPTION 'Regulatory catalog snapshot is incomplete' USING errcode = '23514';
  END IF;
  IF TG_OP = 'UPDATE' AND (OLD.import_status = 'ready' OR OLD.is_canonical) THEN
    RAISE EXCEPTION 'Regulatory catalog versions are immutable' USING errcode = '55000';
  END IF;
...
BEGIN
  IF TG_OP <> 'INSERT' THEN
    RAISE EXCEPTION 'Regulatory catalog rows are immutable' USING errcode = '55000';
  END IF;
  RETURN NEW;
END;
$$;

CREATE TRIGGER device_quota_regulatory_catalog_versions_guard BEFORE INSERT OR UPDATE OR DELETE ON public.device_quota_regulatory_catalog_versions FOR EACH ROW EXECUTE FUNCTION public.device_quota_regulatory_ready_transition();
CREATE TRIGGER device_quota_regulatory_documents_immutable BEFORE UPDATE OR DELETE ON public.device_quota_regulatory_documents FOR EACH ROW EXECUTE FUNCTION public.device_quota_regulatory_immutable_row();
CREATE TRIGGER device_quota_regulatory_sections_immutable BEFORE UPDATE OR DELETE ON public.device_quota_regulatory_sections FOR EACH ROW EXECUTE FUNCTION public.device_quota_regulatory_immutable_row();
CREATE TRIGGER device_quota_regulatory_items_immutable BEFORE UPDATE OR DELETE ON public.device_quota_regulatory_items FOR EACH ROW EXECUTE FUNCTION public.device_quota_regulatory_immutable_row();
CREATE TRIGGER device_quota_regulatory_rules_immutable BEFORE UPDATE OR DELETE ON public.device_quota_regulatory_rules FOR EACH ROW EXECUTE FUNCTION public.device_quota_regulatory_immutable_row();
CREATE TRIGGER device_quota_regulatory_positions_immutable BEFORE UPDATE OR DELETE ON public.device_quota_regulatory_source_positions FOR EACH ROW EXECUTE FUNCTION public.device_quota_regulatory_immutable_row();
CREATE TRIGGER device_quota_regulatory_pages_immutable BEFORE UPDATE OR DELETE ON public.device_quota_regulatory_source_pages FOR EACH ROW EXECUTE FUNCTION public.device_quota_regulatory_immutable_row();
CREATE TRIGGER device_quota_regulatory_references_immutable BEFORE UPDATE OR DELETE ON public.device_quota_regulatory_references FOR EACH ROW EXECUTE FUNCTION public.device_quota_regulatory_immutable_row();
...
CREATE POLICY device_quota_regulatory_documents_no_client_access ON public.device_quota_regulatory_documents FOR ALL TO public USING (false) WITH CHECK (false);
CREATE POLICY device_quota_regulatory_catalog_versions_no_client_access ON public.device_quota_regulatory_catalog_versions FOR ALL TO public USING (false) WITH CHECK (false);
CREATE POLICY device_quota_regulatory_sections_no_client_access ON public.device_quota_regulatory_sections FOR ALL TO public USING (false) WITH CHECK (false);
CREATE POLICY device_quota_regulatory_items_no_client_access ON public.device_quota_regulatory_items FOR ALL TO public USING (false) WITH CHECK (false);
CREATE POLICY device_quota_regulatory_rules_no_client_access ON public.device_quota_regulatory_rules FOR ALL TO public USING (false) WITH CHECK (false);
CREATE POLICY device_quota_regulatory_positions_no_client_access ON public.device_quota_regulatory_source_positions FOR ALL TO public USING (false) WITH CHECK (false);
CREATE POLICY device_quota_regulatory_pages_no_client_access ON public.device_quota_regulatory_source_pages FOR ALL TO public USING (false) WITH CHECK (false);
CREATE POLICY device_quota_regulatory_references_no_client_access ON public.device_quota_regulatory_references FOR ALL TO public USING (false) WITH CHECK (false);

REVOKE ALL ON TABLE public.device_quota_regulatory_documents, public.device_quota_regulatory_catalog_versions, public.device_quota_regulatory_sections, public.device_quota_regulatory_items, public.device_quota_regulatory_rules, public.device_quota_regulatory_source_positions, public.device_quota_regulatory_source_pages, public.device_quota_regulatory_references FROM PUBLIC, anon, authenticated;

DO $seed$
DECLARE
  v_document_id UUID; v_version_id UUID; v_section_id UUID; v_item_id UUID; v_position_id UUID; v_row JSONB; v_page INTEGER; v_line TEXT; v_ordinality INTEGER; v_source_order INTEGER := 0;
...
BEGIN
  INSERT INTO public.device_quota_regulatory_documents (document_number, document_title, appendix_title, document_version, issued_date, effective_date, source_pdf_path, source_pdf_sha256)
  VALUES ('10/2026/TT-BYT','Thông tư 10/2026/TT-BYT','Phụ lục - Tiêu chuẩn, định mức sử dụng máy móc, thiết bị chuyên dùng trong lĩnh vực y tế','2026-05-14','2026-05-14','2026-07-01','757_Thong-tu-10-2026-TT-BYT_88e68354fb.pdf','04186bd3cc50cf541f5e481d25480741412cfe3c899040c35713d4eeda24fd8f') RETURNING id INTO v_document_id;
  INSERT INTO public.device_quota_regulatory_catalog_versions (document_id, artifact_id, appendix_json_path, appendix_json_sha256, appendix_markdown_path, appendix_markdown_sha256, extraction_revision, import_status, is_canonical, source_pages, source_note, expected_structural_rows, expected_section_rows, expected_item_rows, expected_child_item_rows, expected_top_level_item_rows, expected_rule_lines, expected_footnotes, expected_items_with_source_pages, expected_items_with_source_references, expected_multiline_items)
  VALUES (v_document_id,'thong-tu-10-2026-appendix-freeze','thong-tu-10-2026-appendix.json','01aac96335d83fd51ca45e9bce0b03c20ec3a333822151f320ac22948cc9b438','thong-tu-10-2026-appendix.md','1ac4b38c14675b2de065c13c09036b89c92de87c17da840776e351f67761c4ca','phase-0-2026-08-31-r1','loading',false,'6-12','The PDF is the legal ground truth. JSON and Markdown are structural transcriptions used for traceable source inspection.',42,5,37,16,21,113,3,37,37,32) RETURNING id INTO v_version_id;
  FOR v_row IN SELECT value FROM jsonb_array_elements(v_appendix->'rows') WITH ORDINALITY AS rows(value, ordinality) ORDER BY rows.ordinality LOOP
    v_source_order := v_source_order + 1; v_section_id := NULL; v_item_id := NULL;
    IF v_row->>'type' = 'section' THEN
      INSERT INTO public.device_quota_regulatory_sections (catalog_version_id, source_identifier, source_label, name, source_order) VALUES (v_version_id,v_row->>'id',v_row->>'tt',v_row->>'name',v_source_order) RETURNING id INTO v_section_id;
    ELSE
      SELECT s.id INTO v_section_id FROM public.device_quota_regulatory_sections s WHERE s.catalog_version_id=v_version_id AND s.source_identifier=v_row->>'parent';
      INSERT INTO public.device_quota_regulatory_items (catalog_version_id, section_id, source_identifier, source_label, name, original_unit, quota_lines, source_order) VALUES (v_version_id,v_section_id,v_row->>'id',v_row->>'tt',v_row->>'name',v_row->>'unit',COALESCE(ARRAY(SELECT jsonb_array_elements_text(v_row->'quota')),'{}'::TEXT[]),v_source_order) RETURNING id INTO v_item_id;
      FOR v_line, v_ordinality IN SELECT value, ordinality::INTEGER FROM jsonb_array_elements_text(v_row->'quota') WITH ORDINALITY AS quota(value, ordinality) LOOP
        INSERT INTO public.device_quota_regulatory_rules (item_id,line_order,source_text) VALUES (v_item_id,v_ordinality,v_line);
      END LOOP;
    END IF;
    INSERT INTO public.device_quota_regulatory_source_positions (catalog_version_id,source_identifier,source_label,row_type,source_level,parent_source_identifier,source_order,section_id,item_id) VALUES (v_version_id,v_row->>'id',v_row->>'tt',v_row->>'type',(v_row->>'level')::INTEGER,v_row->>'parent',v_source_order,v_section_id,v_item_id) RETURNING id INTO v_position_id;
    FOR v_page IN SELECT value::INTEGER FROM jsonb_array_elements_text(v_row->'source_pages') LOOP
      INSERT INTO public.device_quota_regulatory_source_pages (source_position_id,page_number,page_order) VALUES (v_position_id,v_page,(SELECT count(*)::INTEGER+1 FROM public.device_quota_regulatory_source_pages WHERE source_position_id=v_position_id));
    END LOOP;
    INSERT INTO public.device_quota_regulatory_references (catalog_version_id,source_position_id,reference_type,reference_order,reference_text) VALUES (v_version_id,v_position_id,'source',v_source_order,v_row->>'source_ref');
  END LOOP;
  FOR v_line, v_ordinality IN SELECT value, ordinality::INTEGER FROM jsonb_array_elements_text(v_appendix->'footnotes') WITH ORDINALITY AS footnotes(value, ordinality) LOOP
    INSERT INTO public.device_quota_regulatory_references (catalog_version_id,source_position_id,reference_type,reference_order,reference_text) VALUES (v_version_id,NULL,'footnote',v_ordinality,v_line);
  END LOOP;
  UPDATE public.device_quota_regulatory_catalog_versions SET import_status='ready',is_canonical=true WHERE id=v_version_id;
END;
$seed$;
...
DECLARE
  v_claims JSONB; v_role TEXT; v_effective_role TEXT; v_user_id BIGINT; v_don_vi BIGINT; v_user_id_text TEXT; v_don_vi_text TEXT; v_version_id UUID; v_canonical_count INTEGER;
BEGIN
  BEGIN
    v_claims := COALESCE(NULLIF(current_setting('request.jwt.claims', true), ''), '{}')::JSONB;
    v_role := NULLIF(v_claims->>'app_role',''); v_user_id_text := NULLIF(v_claims->>'user_id',''); v_don_vi_text := NULLIF(v_claims->>'don_vi','');
  EXCEPTION WHEN OTHERS THEN RAISE EXCEPTION 'Missing or malformed JWT claims' USING errcode='42501'; END;
  IF v_role IS NULL OR v_user_id_text IS NULL THEN RAISE EXCEPTION 'Missing authenticated identity claims' USING errcode='42501'; END IF;
  v_effective_role := CASE WHEN v_role='admin' THEN 'global' ELSE v_role END;
  IF v_effective_role IS NULL OR v_effective_role NOT IN ('global','to_qltb') THEN RAISE EXCEPTION 'Insufficient permissions for regulatory catalog access' USING errcode='42501'; END IF;
  IF v_user_id_text !~ '^[0-9]+$' THEN RAISE EXCEPTION 'Missing user_id claim' USING errcode='42501'; END IF;
  IF v_don_vi_text IS NULL OR v_don_vi_text !~ '^[0-9]+$' THEN RAISE EXCEPTION 'Missing tenant claim' USING errcode='42501'; END IF;
  BEGIN v_user_id:=v_user_id_text::BIGINT; v_don_vi:=v_don_vi_text::BIGINT; EXCEPTION WHEN numeric_value_out_of_range THEN RAISE EXCEPTION 'Invalid session identity claims' USING errcode='42501'; END;
  IF NOT EXISTS (SELECT 1 FROM public.nhan_vien nv WHERE nv.id=v_user_id AND COALESCE(nv.is_active,true) AND COALESCE(nv.current_don_vi,nv.don_vi)=v_don_vi AND CASE WHEN nv.role='admin' THEN 'global' ELSE nv.role END=v_effective_role) THEN RAISE EXCEPTION 'Session user or tenant is not authorized' USING errcode='42501'; END IF;
  SELECT count(*)::INTEGER INTO v_canonical_count FROM public.device_quota_regulatory_catalog_versions v JOIN public.device_quota_regulatory_documents d ON d.id=v.document_id WHERE d.document_number='10/2026/TT-BYT' AND v.import_status='ready' AND v.is_canonical AND device_quota_internal.catalog_is_complete(v.id);
  IF v_canonical_count<>1 THEN RAISE EXCEPTION 'Canonical regulatory catalog snapshot is unavailable or invalid' USING errcode='55000'; END IF;
  SELECT v.id INTO v_version_id FROM public.device_quota_regulatory_catalog_versions v JOIN public.device_quota_regulatory_documents d ON d.id=v.document_id WHERE d.document_number='10/2026/TT-BYT' AND v.import_status='ready' AND v.is_canonical AND device_quota_internal.catalog_is_complete(v.id) LIMIT 1;
  RETURN (SELECT jsonb_build_object('document',jsonb_build_object('document_number',d.document_number,'document_title',d.document_title,'appendix_title',d.appendix_title,'document_version',d.document_version,'issued_date',d.issued_date,'effective_date',d.effective_date,'source_pdf_path',d.source_pdf_path,'source_pdf_sha256',d.source_pdf_sha256),'catalog_version',jsonb_build_object('artifact_id',v.artifact_id,'appendix_json_path',v.appendix_json_path,'appendix_json_sha256',v.appendix_json_sha256,'appendix_markdown_path',v.appendix_markdown_path,'appendix_markdown_sha256',v.appendix_markdown_sha256,'extraction_revision',v.extraction_revision,'import_status',v.import_status,'is_canonical',v.is_canonical,'source_pages',v.source_pages,'source_note',v.source_note),'completeness',jsonb_build_object('structural_rows',v.expected_structural_rows,'section_rows',v.expected_section_rows,'equipment_item_rows',v.expected_item_rows,'source_declared_child_rows',v.expected_child_item_rows,'top_level_item_rows',v.expected_top_level_item_rows,'rule_lines',v.expected_rule_lines,'footnotes',v.expected_footnotes,'items_with_source_pages',v.expected_items_with_source_pages,'items_with_source_references',v.expected_items_with_source_references,'multiline_quota_items',v.expected_multiline_items),'rows',(SELECT COALESCE(jsonb_agg(jsonb_build_object('id',p.source_identifier,'tt',p.source_label,'type',p.row_type,'level',p.source_level,'parent',p.parent_source_identifier,'name',COALESCE(s.name,i.name),'unit',i.original_unit,'quota',CASE WHEN i.id IS NULL THEN NULL ELSE to_jsonb(i.quota_lines) END,'source_pages',(SELECT jsonb_agg(sp.page_number ORDER BY sp.page_order) FROM public.device_quota_regulatory_source_pages sp WHERE sp.source_position_id=p.id),'source_ref',(SELECT r.reference_text FROM public.device_quota_regulatory_references r WHERE r.source_position_id=p.id AND r.reference_type='source') ) ORDER BY p.source_order),'[]'::JSONB) FROM public.device_quota_regulatory_source_positions p LEFT JOIN public.device_quota_regulatory_sections s ON s.id=p.section_id LEFT JOIN public.device_quota_regulatory_items i ON i.id=p.item_id WHERE p.catalog_version_id=v.id),'footnotes',(SELECT COALESCE(jsonb_agg(r.reference_text ORDER BY r.reference_order),'[]'::JSONB) FROM public.device_quota_regulatory_references r WHERE r.catalog_version_id=v.id AND r.reference_type='footnote')) FROM public.device_quota_regulatory_catalog_versions v JOIN public.device_quota_regulatory_documents d ON d.id=v.document_id WHERE v.id=v_version_id);
END;
$$;

REVOKE ALL ON FUNCTION public.device_quota_regulatory_catalog_get() FROM PUBLIC, anon, authenticated, service_role;
