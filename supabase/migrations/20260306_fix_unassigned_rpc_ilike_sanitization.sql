-- Fix: Apply _sanitize_ilike_pattern to dinh_muc_thiet_bi_unassigned
-- The search parameter was concatenated directly into a LIKE pattern
-- without escaping %, _, and \ — causing _ to match any character
-- and % to match everything. This is the only remaining RPC without
-- the fix (all others were patched in earlier migrations).
...
BEGIN
  ...
  -- FIX: Use _sanitize_ilike_pattern instead of raw concatenation
