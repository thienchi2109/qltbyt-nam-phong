...
-- Migration Date: 2025-10-04 08:00 UTC
  ...
  ELSE
    ...
    IF v_allowed_don_vi IS NOT NULL AND array_length(v_allowed_don_vi, 1) > 0 THEN
      IF p_don_vi IS NOT NULL THEN
        ...
        END IF;
      ELSE
        -- Use first allowed don_vi for single-tenant queries
        v_effective_donvi := v_allowed_don_vi[1];
      END IF;
    ELSE
      -- No access
      RETURN jsonb_build_object(
...
BEGIN
  ...
  CASE lower(p_filter_type)
    WHEN 'khoa_phong' THEN
      EXECUTE format('
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object('value', COALESCE(NULLIF(TRIM(khoa_phong_quan_ly), ''''), ''Chưa phân loại''), 'label', COALESCE(NULLIF(TRIM(khoa_phong_quan_ly), ''''), ''Chưa phân loại''))
        ), ''[]''::jsonb) FROM (
    ...
    WHEN 'nguoi_su_dung' THEN
      EXECUTE format('
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object('value', COALESCE(NULLIF(TRIM(nguoi_dang_truc_tiep_quan_ly), ''''), ''Chưa phân loại''), 'label', COALESCE(NULLIF(TRIM(nguoi_dang_truc_tiep_quan_ly), ''''), ''Chưa phân loại''))
        ), ''[]''::jsonb) FROM (
    ...
    WHEN 'vi_tri_lap_dat' THEN
      EXECUTE format('
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object('value', COALESCE(NULLIF(TRIM(vi_tri_lap_dat), ''''), ''Chưa phân loại''), 'label', COALESCE(NULLIF(TRIM(vi_tri_lap_dat), ''''), ''Chưa phân loại''))
        ), ''[]''::jsonb) FROM (
    ...
    WHEN 'tinh_trang' THEN
      EXECUTE format('
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object('value', COALESCE(NULLIF(TRIM(tinh_trang_hien_tai), ''''), ''Chưa phân loại''), 'label', COALESCE(NULLIF(TRIM(tinh_trang_hien_tai), ''''), ''Chưa phân loại''))
        ), ''[]''::jsonb) FROM (
    ...
    WHEN 'phan_loai' THEN
      EXECUTE format('
        SELECT COALESCE(jsonb_agg(
          jsonb_build_object('value', COALESCE(NULLIF(TRIM(phan_loai_theo_nd98), ''''), ''Chưa phân loại''), 'label', COALESCE(NULLIF(TRIM(phan_loai_theo_nd98), ''''), ''Chưa phân loại''))
        ), ''[]''::jsonb) FROM (
