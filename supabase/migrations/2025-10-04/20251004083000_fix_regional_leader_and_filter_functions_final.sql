-- Fix Regional Leader Data Access and Filter Functions - Final Version
-- Issue 1: Regional leaders get "No tenant access" error
-- Issue 2: SQL syntax errors in equipment_filter_options function
-- Issue 3: Sort parsing examples also get "No tenant access" error
-- Solution: Fix SQL syntax and regional leader access logic
-- Migration Date: 2025-10-04 08:30 UTC
  ...
  CASE lower(p_filter_type)
    ...
    ELSE
      ...
      SELECT jsonb_build_object(
        'khoa_phong', (
          SELECT jsonb_agg(jsonb_build_object('value', dept.khoa_phong_quan_ly, 'label', dept.khoa_phong_quan_ly))
          FROM (
            SELECT DISTINCT COALESCE(NULLIF(TRIM(khoa_phong_quan_ly), ''), 'Chưa phân loại') as khoa_phong_quan_ly
            FROM public.thiet_bi
            WHERE v_where
        ...
        'nguoi_su_dung', (
          SELECT jsonb_agg(jsonb_build_object('value', usr.nguoi_dang_truc_tiep_quan_ly, 'label', usr.nguoi_dang_truc_tiep_quan_ly))
          FROM (
            SELECT DISTINCT COALESCE(NULLIF(TRIM(nguoi_dang_truc_tiep_quan_ly), ''), 'Chưa phân loại') as nguoi_dang_truc_tiep_quan_ly
            FROM public.thiet_bi
            WHERE v_where
        ...
        'vi_tri_lap_dat', (
          SELECT jsonb_agg(jsonb_build_object('value', loc.vi_tri_lap_dat, 'label', loc.vi_tri_lap_dat))
          FROM (
            SELECT DISTINCT COALESCE(NULLIF(TRIM(vi_tri_lap_dat), ''), 'Chưa phân loại') as vi_tri_lap_dat
            FROM public.thiet_bi
            WHERE v_where
        ...
        'tinh_trang', (
          SELECT jsonb_agg(jsonb_build_object('value', st.tinh_trang_hien_tai, 'label', st.tinh_trang_hien_tai))
          FROM (
            SELECT DISTINCT COALESCE(NULLIF(TRIM(tinh_trang_hien_tai), ''), 'Chưa phân loại') as tinh_trang_hien_tai
            FROM public.thiet_bi
            WHERE v_where
        ...
        'phan_loai', (
          SELECT jsonb_agg(jsonb_build_object('value', cls.phan_loai_theo_nd98, 'label', cls.phan_loai_theo_nd98))
          FROM (
            SELECT DISTINCT COALESCE(NULLIF(TRIM(phan_loai_theo_nd98), ''), 'Chưa phân loại') as phan_loai_theo_nd98
            FROM public.thiet_bi
            WHERE v_where
