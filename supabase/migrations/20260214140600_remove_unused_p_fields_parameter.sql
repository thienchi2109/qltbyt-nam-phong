...
-- Drop the old overload that included p_fields as the last parameter.
-- CREATE OR REPLACE cannot replace a function with a different signature;
-- without this DROP the old overload would persist alongside the new one,
-- allowing callers to keep passing p_fields and getting silently wrong results.
...
Note: p_fields parameter removed — field filtering was never implemented.';
