-- Fix ambiguous dia_ban_id reference inside authenticate_user_dual_mode
-- Ensures column lookups are fully qualified after adding dia_ban return fields
-- ROLLBACK: Re-run the original definition from 20250927_global_admin_user_management.sql if needed
...
BEGIN
  ...
  IF v_resolved_don_vi IS NOT NULL THEN
    SELECT
      dv_lookup.active,
      dv_lookup.dia_ban_id
    INTO v_tenant_active, v_resolved_dia_ban
    FROM public.don_vi AS dv_lookup
    WHERE dv_lookup.id = v_resolved_don_vi
    LIMIT 1;
  ...
  IF v_resolved_dia_ban IS NOT NULL THEN
    SELECT
      db_lookup.ma_dia_ban,
      db_lookup.active
    INTO v_resolved_dia_ban_ma, v_dia_ban_active
    FROM public.dia_ban AS db_lookup
    WHERE db_lookup.id = v_resolved_dia_ban
    LIMIT 1;
