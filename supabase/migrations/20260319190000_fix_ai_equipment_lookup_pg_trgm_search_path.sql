-- Fix ai_equipment_lookup: schema-qualify pg_trgm word_similarity() calls.
--
-- Root cause: pg_trgm is installed in the "extensions" schema (Supabase default),
-- but the function uses SET search_path TO 'public', 'pg_temp' (SECURITY DEFINER).
-- Unqualified word_similarity() is invisible → ERROR 42883 at runtime.
--
-- Fix: prefix the two word_similarity() calls with extensions.word_similarity().
-- This is safer than adding "extensions" to search_path (smaller attack surface).
--
-- Affected lines: department filter (line ~268) and location filter (line ~275)
-- in the original migration 20260316205500.
  ...
  RETURN (
    WITH filtered AS (
      ...
      WHERE tb.is_deleted = FALSE
        ...
        -- FIX: schema-qualify extensions.word_similarity (pg_trgm lives in extensions schema)
        AND (
          ...
          OR (v_department_filter_raw IS NOT NULL
              AND extensions.word_similarity(v_department_filter_raw, COALESCE(tb.khoa_phong_quan_ly, '')) > 0.4)
        )
        ...
        AND (
          ...
          OR (v_location_filter_raw IS NOT NULL
              AND extensions.word_similarity(v_location_filter_raw, COALESCE(tb.vi_tri_lap_dat, '')) > 0.4)
        )
        AND (
