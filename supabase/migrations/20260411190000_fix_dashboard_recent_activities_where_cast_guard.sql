-- Force safe tenant_don_vi evaluation order in dashboard_recent_activities.
-- PostgreSQL may reorder boolean predicates, so the WHERE clause must use
-- CASE instead of relying on regex AND cast short-circuiting.
...
BEGIN
  ...
  WHERE al.action_type IN (
    ...
    AND (
      ...
      OR CASE
        WHEN NULLIF(al.tenant_don_vi, '') ~ '^[0-9]+$'
          THEN NULLIF(al.tenant_don_vi, '')::BIGINT = ANY(v_allowed)
        ELSE FALSE
      END
    )
