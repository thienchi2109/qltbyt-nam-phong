-- Re-assert the safe dashboard_recent_activities RPC after the malformed
-- 20260411_dashboard_recent_activities_rpc.sql filename. This keeps fresh
-- bootstrap environments on the safe function body even if the legacy file
-- sorts unexpectedly during migration discovery.
...
DECLARE
  v_role_raw    TEXT := COALESCE(
                      NULLIF(public._get_jwt_claim('app_role'), ''),
                      NULLIF(public._get_jwt_claim('role'), ''),
                      '');
  v_role        TEXT := lower(v_role_raw);
  v_user_id     TEXT := COALESCE(
                      NULLIF(public._get_jwt_claim('user_id'), ''),
                      NULLIF(public._get_jwt_claim('sub'), ''));
  v_don_vi_raw  TEXT := NULLIF(public._get_jwt_claim('don_vi'), '');
  v_don_vi      BIGINT;
  v_is_global   BOOLEAN;
  v_allowed     BIGINT[];
  v_safe_limit  INT;
BEGIN
  IF v_don_vi_raw ~ '^[0-9]+$' THEN
    v_don_vi := v_don_vi_raw::BIGINT;
  END IF;
...
'Empty-string role and user_id claims fall back to role/sub respectively. '
