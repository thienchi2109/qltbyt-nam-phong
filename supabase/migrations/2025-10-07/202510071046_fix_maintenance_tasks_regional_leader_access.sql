...
-- function allowed_don_vi_for_session() which returns an array of accessible
...
-- ============================================================================
-- ✅ Helper function allowed_don_vi_for_session() exists and returns bigint[]
-- ✅ Helper is SECURITY DEFINER with safe owner (postgres)
-- ✅ Index idx_thiet_bi_don_vi exists on thiet_bi(don_vi)
-- ✅ Index idx_cong_viec_bao_tri_ke_hoach_id exists
-- ✅ Index idx_cong_viec_bao_tri_thiet_bi_id exists
-- ✅ Role literals verified: 'global', 'regional_leader', 'to_qltb', 'user'
-- ============================================================================
...
BEGIN
  -- Use allowed_don_vi_for_session() helper for proper multi-tenant filtering
  ...
  WHERE (p_ke_hoach_id IS NULL OR cv.ke_hoach_id = p_ke_hoach_id)
    ...
    AND (
      ...
      OR tb.don_vi = ANY(v_allowed)  -- ✅ FIXED: Use array check instead of single value
    )
