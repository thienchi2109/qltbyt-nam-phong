-- Follow-up: make header notification counts require a valid equipment row.
-- 20260424094059 already fixed p_don_vi scoping and was applied to production.
-- This patch keeps that contract but changes LEFT JOINs to INNER JOINs so
-- global "all facilities" and selected-facility counts use the same row set.
...
COMMENT ON FUNCTION public.header_notifications_summary(BIGINT) IS
  'Returns pending repair/transfer notification counts for the current JWT scope; p_don_vi narrows counts to an authorized selected facility. Counts require a matching equipment row.';
