-- Dashboard Recent Activities RPC
-- Returns recent notable activities from audit_logs for the dashboard feed.
-- Tenant-isolated: uses JWT claims + allowed_don_vi_for_session_safe().
-- regional_leader sees all facilities in their dia_ban.
-- Includes facility_name for multi-tenant context.
...
BEGIN
  ...
  IF NOT v_is_global THEN
    -- regional_leader may legitimately have don_vi = NULL
    -- because scope comes from dia_ban.
    ...
    IF array_length(v_allowed, 1) IS NULL THEN
      RETURN;  -- no accessible facilities → empty result
    END IF;
  END IF;
  ...
  -- Action type → Vietnamese label mapping (inline CASE for performance)
  RETURN QUERY
  SELECT
    ...
    CASE al.action_type
      ...
      WHEN 'maintenance_plan_approve'     THEN 'Phê duyệt kế hoạch bảo trì'
  ...
  LEFT JOIN public.don_vi dv    ON dv.id = al.tenant_don_vi::BIGINT
  ...
  AND (
    v_is_global
    OR al.tenant_don_vi::BIGINT = ANY(v_allowed)
  )
