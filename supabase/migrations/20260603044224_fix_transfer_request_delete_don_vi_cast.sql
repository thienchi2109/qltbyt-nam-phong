-- Fix transfer_request_delete tenant comparison when JWT don_vi is text and thiet_bi.don_vi is bigint.
...
BEGIN
  ...
  IF v_role IS DISTINCT FROM 'global'
    AND v_don_vi IS NOT NULL
    AND v_req.tb_don_vi::text IS DISTINCT FROM v_don_vi
  THEN
