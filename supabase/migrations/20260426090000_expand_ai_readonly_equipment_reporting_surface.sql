-- Expand ai_readonly.equipment_search with governed reporting-safe dimensions.
-- Purpose: support assistant SQL grouping/filtering by manager, location, ND98,
-- manufacturer/source/funding, and normalized date buckets without granting
-- access to raw public schemas.
-- Rollback (forward-only): re-apply
--   supabase/migrations/20260418120000_add_ai_readonly_semantic_layer_foundation.sql
-- to restore the prior ai_readonly.equipment_search shape, then drop
-- ai_readonly.try_parse_iso_date(text) if no later migration depends on it.
