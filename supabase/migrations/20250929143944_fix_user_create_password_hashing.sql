-- Fix user_create function to hash passwords properly for new users
-- Issue: New users created via user_create have plain text passwords, but authenticate_user_dual_mode 
-- expects hashed passwords first. This causes login failures for newly created accounts.
...
COMMENT ON FUNCTION public.user_create IS 'Creates a new user with properly hashed password - fixed authentication issue';