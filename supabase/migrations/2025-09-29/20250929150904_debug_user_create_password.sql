-- Debug version of user_create function to log what passwords are being received
-- This will help us understand why UI-created users can't login
...
begin
  -- Debug logging
  RAISE NOTICE 'user_create called with username=%, password_length=%, full_name=%, role=%, don_vi=%', 
    p_username, length(p_password), p_full_name, p_role, p_current_don_vi;
  ...
  RAISE NOTICE 'Generated hash for user %: hash_length=%, hash_prefix=%', 
    p_username, length(v_hashed_password), substring(v_hashed_password, 1, 10);
  ...
  RAISE NOTICE 'User % created successfully with ID=%', p_username, v_id;
...
-- Add a debug function to test password hashing directly
CREATE OR REPLACE FUNCTION public.debug_password_hash(p_password text)
RETURNS TABLE(
  original_password text,
  password_length integer,
  generated_hash text,
  hash_length integer,
  test_verification boolean
)
LANGUAGE plpgsql
AS $$
declare
...
begin
  ...
  return query select 
    ...
    v_hash,
    length(v_hash),
    (extensions.crypt(p_password, v_hash) = v_hash) as test_verification;
end;
$$;

GRANT EXECUTE ON FUNCTION public.debug_password_hash(text) TO authenticated;

-- Add comment for documentation
COMMENT ON FUNCTION public.user_create IS 'Creates a new user with debug logging for password investigation';
COMMENT ON FUNCTION public.debug_password_hash IS 'Debug function to test password hashing - remove after issue is resolved';