-- Migration: Restore user information in transfer_request_list_enhanced
-- Date: 2025-10-08 14:45
-- Purpose: Add nguoi_yeu_cau and nguoi_duyet joins to transfer list query
-- Issue: TransferDetailDialog expects nested user objects but RPC was only returning yc.* and thiet_bi
-- MANUAL REVIEW REQUIRED - DO NOT AUTO-APPLY

BEGIN;

-- Replace transfer_request_list_enhanced to include user information
...
-- CHANGE SUMMARY
-- ============================================================================
-- Before: Only returned yc.* and thiet_bi object
-- After:  Also returns nguoi_yeu_cau and nguoi_duyet objects
--
-- Added JOINs:
-- - LEFT JOIN nhan_vien nyc ON yc.nguoi_yeu_cau_id = nyc.id
