-- Issue #646 follow-up: serialize ZBS OAuth token-state bootstrap paths.
--
-- This migration supersedes the already-applied token-state RPC definitions
-- without editing prior applied migrations.
...
BEGIN
  ...
  PERFORM pg_advisory_xact_lock(
    pg_catalog.hashtext('zbs_oauth_token_state:' || v_provider)::bigint
  );
...
COMMENT ON FUNCTION public.zbs_oauth_token_state_persist_success(text, text, text, timestamptz, text, timestamptz, timestamptz) IS
  'Service-role-only atomic ZBS token refresh success boundary. Serializes provider bootstrap, locks existing provider row, and compares the previous refresh token before persisting rotated tokens.';
COMMENT ON FUNCTION public.zbs_oauth_token_state_record_error(text, text, text, text, timestamptz) IS
  'Service-role-only ZBS token refresh error boundary. Serializes provider bootstrap and stores sanitized error metadata only when the refresh token has not rotated.';
