-- Migration: add equipment_bulk_delete RPC for atomic page-scoped soft delete
-- Date: 2026-02-21
-- Notes:
--   - Uses JWT claims for tenant isolation (no p_don_vi override parameter)
--   - Uses deterministic row locking order (ORDER BY id FOR UPDATE) to avoid deadlocks
--   - Emits per-row audit logs with shared batch_id/batch_size for correlation
...
COMMENT ON FUNCTION public.equipment_bulk_delete(BIGINT[]) IS
  'Bulk soft-delete for equipment. No p_don_vi parameter: tenant isolation is enforced from JWT claims.';
