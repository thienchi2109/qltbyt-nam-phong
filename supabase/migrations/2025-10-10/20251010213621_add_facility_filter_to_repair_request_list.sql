-- Add server-side facility filtering to repair_request_list for regional leaders
-- This matches the pattern used in equipment_list_enhanced
-- Eliminates client-side filtering crashes by filtering at the database level

-- Drop old 4-parameter version
DROP FUNCTION IF EXISTS public.repair_request_list(text,text,integer,integer);

-- Create new 5-parameter version with facility filtering
...
BEGIN
  ...
  FROM (
    ...
    WHERE (
      ...
      -- When p_don_vi is provided, filter by that specific facility
...
'Lists repair requests with equipment and facility information. Supports server-side facility filtering via p_don_vi parameter for regional leaders. Returns only requests for equipment in facilities the user has access to.';
