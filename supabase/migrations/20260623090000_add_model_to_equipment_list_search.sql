-- Add model to equipment_list_enhanced search.
--
-- The Equipments page sends its search text as p_q to this RPC. Keep the
-- existing response contract and tenant guards, but include thiet_bi.model in
-- the server-side ILIKE search condition.
--
-- Verification: run
-- supabase/tests/equipment_list_enhanced_model_search_smoke.sql.
--
-- Rollback: re-apply the previous function body from
-- supabase/migrations/20260428040500_fix_equipment_list_enhanced_active_repair_contract.sql.

BEGIN;

CREATE EXTENSION IF NOT EXISTS pg_trgm;

CREATE INDEX IF NOT EXISTS idx_thiet_bi_model_trgm
ON public.thiet_bi USING gin (model gin_trgm_ops);
