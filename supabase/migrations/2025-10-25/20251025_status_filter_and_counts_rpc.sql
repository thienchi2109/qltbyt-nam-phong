-- Server-side status multi-select + status counts RPC for repair requests
-- IMPORTANT: This migration extends repair_request_list and adds repair_request_status_counts.

-- Drop old 7-arg version to avoid overload confusion (safe if not present)
DROP FUNCTION IF EXISTS public.repair_request_list(TEXT, TEXT, INT, INT, BIGINT, DATE, DATE);
...
COMMENT ON FUNCTION public.repair_request_list IS 'Lists repair requests with facility scoping, pagination, VN-local date range filtering, optional search, and multi-status filtering (p_statuses).';

-- Status counts RPC (excludes status to reflect overall totals for current non-status filters)
...
BEGIN
  ...
  ELSE
    v_allowed := public.allowed_don_vi_for_session();
    IF v_allowed IS NULL OR array_length(v_allowed, 1) IS NULL THEN
      RETURN jsonb_build_object('Chờ xử lý',0,'Đã duyệt',0,'Hoàn thành',0,'Không HT',0);
    END IF;
    IF p_don_vi IS NOT NULL AND NOT (p_don_vi = ANY(v_allowed)) THEN
      RETURN jsonb_build_object('Chờ xử lý',0,'Đã duyệt',0,'Hoàn thành',0,'Không HT',0);
    END IF;
    v_effective_donvi := p_don_vi; -- may be null => use allowed list
  END IF;
...
COMMENT ON FUNCTION public.repair_request_status_counts IS 'Returns counts per status for current filters (q, date range, facility), excluding status filter; respects tenant isolation and role-based scoping.';
