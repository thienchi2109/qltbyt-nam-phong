-- Part A: ai_department_list – return all distinct departments (khoa/phòng)
-- that have equipment for the scoped facility/facilities.
-- Used by the AI departmentList tool so the model picks exact department
-- names before calling equipmentLookup.
--
-- Part B: ai_equipment_lookup – add pg_trgm word_similarity fallback
-- for department and location filters so that fuzzy/prefix-mismatch
-- queries (e.g. "Khoa Xét Nghiệm" vs DB "Xét Nghiệm") still match.
--
...
-- ═══════════════════════════════════════════════════════════════════
-- PART A: ai_department_list
-- ═══════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public.ai_department_list(
...
BEGIN
  ...
  -- Tenant isolation (same pattern as ai_category_list)
  IF v_is_global THEN
  ...
  -- Return distinct department names with equipment count
  RETURN (
    WITH depts AS (
      SELECT
        khoa_phong_quan_ly AS name,
        COUNT(*) AS equipment_count
      FROM public.thiet_bi
      WHERE is_deleted = FALSE
        AND khoa_phong_quan_ly IS NOT NULL
        AND BTRIM(khoa_phong_quan_ly) <> ''
        AND (v_effective IS NULL OR don_vi = ANY(v_effective))
      GROUP BY khoa_phong_quan_ly
      ORDER BY khoa_phong_quan_ly ASC
    )
...
GRANT EXECUTE ON FUNCTION public.ai_department_list(BIGINT, TEXT) TO authenticated;
REVOKE EXECUTE ON FUNCTION public.ai_department_list(BIGINT, TEXT) FROM PUBLIC;

-- ═══════════════════════════════════════════════════════════════════
-- PART B: ai_equipment_lookup – add pg_trgm word_similarity fallback
-- for department and location filters.
-- Full CREATE OR REPLACE to ensure idempotency.
-- ═══════════════════════════════════════════════════════════════════
...
DECLARE
  ...
  v_equipment_code_filter TEXT := NULLIF(BTRIM(COALESCE(v_filters->>'equipmentCode', v_filters->>'ma_thiet_bi', v_filters->>'deviceCode')), '');
  ...
  RETURN (
    WITH filtered AS (
      ...
      WHERE tb.is_deleted = FALSE
        ...
        AND (
          v_equipment_code_filter IS NULL
          OR tb.ma_thiet_bi = v_equipment_code_filter
        )
        AND (
          v_status_filter IS NULL
          OR tb.tinh_trang_hien_tai = v_status_filter
        )
        -- Department filter: ILIKE + pg_trgm word_similarity fallback
        AND (
          v_department_filter IS NULL
          OR COALESCE(tb.khoa_phong_quan_ly, '') ILIKE '%' || v_department_filter || '%'
          OR (v_department_filter_raw IS NOT NULL
              AND word_similarity(v_department_filter_raw, COALESCE(tb.khoa_phong_quan_ly, '')) > 0.4)
        )
        -- Location filter: ILIKE + pg_trgm word_similarity fallback
        AND (
          v_location_filter IS NULL
          OR COALESCE(tb.vi_tri_lap_dat, '') ILIKE '%' || v_location_filter || '%'
          OR (v_location_filter_raw IS NOT NULL
              AND word_similarity(v_location_filter_raw, COALESCE(tb.vi_tri_lap_dat, '')) > 0.4)
        )
        AND (
    ...
    SELECT jsonb_build_object(
      ...
      jsonb_strip_nulls(
        jsonb_build_object(
          'equipmentCode', v_equipment_code_filter,
