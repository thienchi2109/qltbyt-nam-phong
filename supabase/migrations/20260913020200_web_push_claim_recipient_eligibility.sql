-- Recheck recipient eligibility before materialization and every lease/retry.
BEGIN;

CREATE OR REPLACE FUNCTION public.web_push_delivery_claim(p_request jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
    ...
    SELECT public.web_push_subject_can_receive(v_intent.recipient_user_id, v_intent.don_vi_id, v_intent.request_id)
      ...
      AND EXISTS (
        ...
        WHERE cfg.don_vi_id = v_intent.don_vi_id AND cfg.user_id = v_intent.recipient_user_id
          AND public.web_push_recipient_is_eligible(cfg.user_id, cfg.don_vi_id, cfg.protected_by_user_id)
      )
