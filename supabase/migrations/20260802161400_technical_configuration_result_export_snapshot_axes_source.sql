-- P14A4: preserve the P14A1 token payload and expose its ordered descriptors
-- only to private downstream export sources. The public manifest remains exact.
-- Rollback with a new reviewed migration that restores the P14A2 helper body.
BEGIN;
...
BEGIN
  ...
  RETURN v_snapshot_data || jsonb_build_object(
    'options', v_snapshot_payload->'options',
    'criteria', v_snapshot_payload->'criteria',
    'snapshot_token',
