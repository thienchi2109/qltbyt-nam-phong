-- Fix repairs RPCs: cast JWT don_vi to BIGINT to match thiet_bi.don_vi
-- Safe: CREATE OR REPLACE only; no signature changes
...
as $$
declare
  v_claims jsonb; v_role text; v_don_vi bigint; v_req yeu_cau_sua_chua; v_tb_don_vi bigint;
begin
  v_claims := coalesce(current_setting('request.jwt.claims', true), '{}')::jsonb;
  v_role := coalesce(nullif(v_claims->>'app_role',''), nullif(v_claims->>'role',''));
  v_don_vi := nullif(v_claims->>'don_vi','')::bigint;
...
security definer
as $$
declare v_req yeu_cau_sua_chua; v_claims jsonb; v_role text; v_don_vi bigint; v_tb record;
begin
  v_claims := coalesce(current_setting('request.jwt.claims', true), '{}')::jsonb;
  v_role := coalesce(nullif(v_claims->>'app_role',''), nullif(v_claims->>'role',''));
  v_don_vi := nullif(v_claims->>'don_vi','')::bigint;
...
declare 
  ...
  v_status text; 
  v_claims jsonb; v_role text; v_don_vi bigint; v_tb record;
begin
  v_claims := coalesce(current_setting('request.jwt.claims', true), '{}')::jsonb;
  v_role := coalesce(nullif(v_claims->>'app_role',''), nullif(v_claims->>'role',''));
  v_don_vi := nullif(v_claims->>'don_vi','')::bigint;
...
security definer
as $$
declare v_req yeu_cau_sua_chua; v_claims jsonb; v_role text; v_don_vi bigint; v_tb_don_vi bigint;
begin
  v_claims := coalesce(current_setting('request.jwt.claims', true), '{}')::jsonb;
  v_role := coalesce(nullif(v_claims->>'app_role',''), nullif(v_claims->>'role',''));
  v_don_vi := nullif(v_claims->>'don_vi','')::bigint;
