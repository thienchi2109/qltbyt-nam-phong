-- Chunk 2: explicit tri-state specialty filtering; preserve the existing four-argument RPC.
BEGIN;
...
BEGIN
  ...
  IF p_filter_specialty IS NULL
     OR (NOT p_filter_specialty AND p_specialty IS NOT NULL)
     OR char_length(public._technical_configuration_normalize_specialty(p_specialty)) > 200
     OR (p_specialty IS NOT NULL AND public._technical_configuration_normalize_specialty(p_specialty) IS NULL)
     OR p_page IS NULL
  ...
  candidate_dossiers AS MATERIALIZED (
    ...
    WHERE (p_include_archived OR d.archived_at IS NULL)
      AND (NOT p_filter_specialty OR lower(d.specialty) IS NOT DISTINCT FROM lower(v_specialty))
      AND (
...
REVOKE ALL ON FUNCTION public.technical_configuration_dossiers_list(INTEGER, INTEGER, BOOLEAN, TEXT, BOOLEAN, TEXT)
  FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.technical_configuration_dossiers_list(INTEGER, INTEGER, BOOLEAN, TEXT, BOOLEAN, TEXT)
  TO authenticated;
...
SET search_path = public, pg_temp
AS $$
  SELECT public.technical_configuration_dossiers_list(
    p_page, p_page_size, p_include_archived, p_search, false, NULL
  );
$$;
