-- Add read-only AI device quota lookup RPC with strict JWT guardrails.
-- For a specific device, returns its quota status within the facility's
-- active decision: inQuotaCatalog, notMapped, notInApprovedCatalog,
...
BEGIN
  ...
  -- ── Step 1: Check if device is mapped to a category ───────────
  ...
  -- ── Step 2: Find active decision for the device's facility ────
  ...
  -- ── Step 5: Count devices in same category ────────────────────
  ...
  -- ── Step 6: Return full quota status ──────────────────────────
  RETURN jsonb_build_object(
    'kind', 'deviceQuotaLookup',
    'device', jsonb_build_object(
      'id', ai_device_quota_lookup.thiet_bi_id,
