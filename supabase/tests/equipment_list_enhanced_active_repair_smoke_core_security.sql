...
BEGIN
  ...
  IF v_row IS NULL OR (v_row->>'id')::bigint IS DISTINCT FROM v_eq_b THEN
    RAISE EXCEPTION 'Scenario G failed: global must receive tenant B equipment %, got %', v_eq_b, v_row;
  END IF;
