...
func TestPausedWorkerPreflightMarksLocalReadinessWithoutClaim(t *testing.T) {
	key := testVAPIDKey(t)
	now := time.Date(2026, 9, 16, 12, 0, 0, 0, time.UTC)
	api := &fakeAPI{}
	state := NewHealthState(true, NewMetrics())
	worker := NewWorker(WorkerConfig{
		API:                 api,
		Sender:              &fakeSender{},
		VAPID:               key,
	...
	if api.claimCalls != 0 {
		t.Fatalf("claim calls = %d, want 0 during paused preflight", api.claimCalls)
	}

	recorder := httptest.NewRecorder()
	state.Handler().ServeHTTP(recorder, httptest.NewRequest(http.MethodGet, "/readyz", nil))
	if recorder.Code != http.StatusOK || recorder.Body.String() != "paused\n" {
		t.Fatalf("paused /readyz = %d %q, want 200 paused", recorder.Code, recorder.Body.String())
	}
}
...
func TestHealthReadinessAndMetricsExposeOnlyBoundedLocalState(t *testing.T) {
	...
	metrics.Record("not_sent_lease_expired", 3*time.Second)
	...
	assertHTTP("/readyz", http.StatusOK, "paused\n")
	...
	for _, expected := range []string{
		...
		"web_push_deliveries_retried_total 0\n",
		"web_push_deliveries_retryable_total 1\n",
		"web_push_deliveries_backend_owned_total 1\n",
		"web_push_deliveries_expired_total 1\n",
		"web_push_send_latency_seconds_count 4\n",
	} {
...
func TestMetricsDoNotReportRetryBeforeItOccurs(t *testing.T) {
	metrics := NewMetrics()
	metrics.Record("transient", 0)
	metrics.Record("not_sent_lease_expired", 0)
