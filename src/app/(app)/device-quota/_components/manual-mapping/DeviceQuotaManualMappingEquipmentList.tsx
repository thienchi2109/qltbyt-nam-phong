...
import type { DeviceQuotaManualMappingEquipmentListProps } from "./DeviceQuotaManualMappingTypes"

/**
 * Route-agnostic list of unassigned equipment for manual quota mapping.
 *
 * Features:
 ...
 */
export function DeviceQuotaManualMappingEquipmentList({
  unassignedEquipment,
  totalEquipmentCount,
  ...
  isLoading,
  isFacilitySelected,
}: DeviceQuotaManualMappingEquipmentListProps) {
    ...
    <Card className="flex h-full min-h-0 flex-col overflow-hidden">
      <CardHeader className="pb-4">
        <ListFilterSearchCard
          ...
          searchPlaceholder={
            isFacilitySelected ? "Tìm kiếm thiết bị..." : "Chọn cơ sở để tìm kiếm..."
          }
            ...
            <div className="flex items-center gap-4">
              <DataTablePaginationSizeSelector
                ...
                onPageSizeChange={(size) =>
                  pagination.setPagination({ pageIndex: 0, pageSize: size })
                }
              />
              <DataTablePaginationNavigation
                ...
                onPreviousPage={() =>
                  pagination.setPagination((prev) => ({ ...prev, pageIndex: prev.pageIndex - 1 }))
                }
                onNextPage={() =>
                  pagination.setPagination((prev) => ({ ...prev, pageIndex: prev.pageIndex + 1 }))
                }
                ...
                onPageJump={(page) =>
                  pagination.setPagination((prev) => ({ ...prev, pageIndex: page - 1 }))
                }
              />
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="font-medium text-sm leading-tight">{equipment.ten_thiet_bi}</div>
        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-muted-foreground">
          <span className="font-mono">{equipment.ma_thiet_bi}</span>
          {equipment.model && <span className="truncate">Model: {equipment.model}</span>}
        </div>
        {equipment.khoa_phong_quan_ly && (
          <div className="text-xs text-muted-foreground">Khoa: {equipment.khoa_phong_quan_ly}</div>
        )}
      </div>
    </button>
  )
...
function NoResultsState() {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <p className="text-sm text-muted-foreground max-w-sm">
        Không có thiết bị nào khớp với bộ lọc hoặc từ khóa hiện tại. Hãy điều chỉnh bộ lọc hoặc thay
