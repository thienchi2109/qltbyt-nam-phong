...
function DeviceNameRow({
  ...
  return (
    ...
    >
      <span className={cn("text-sm truncate", isExcluded && "line-through")}>{name}</span>
      <Button
        variant="ghost"
        size="icon"
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            <span className="text-sm font-semibold truncate block">{group.nhom_label}</span>
            <div className="flex items-center gap-1.5 mt-0.5">
              <Badge variant="secondary" className="font-mono text-xs">
                {group.nhom_code}
              </Badge>
              <span className="text-xs text-muted-foreground">{totalDeviceCount} thiết bị</span>
            </div>
          </div>
        </div>
