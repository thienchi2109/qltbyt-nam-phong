...
} from "../mapping-preview/MappingPreviewPrimitives"
...
function SuggestedMappingPreviewDialogContent({
  ...
  const [excludedDeviceNames, setExcludedDeviceNames] = React.useState<Map<number, Set<string>>>(
    new Map()
  )
  ...
  React.useEffect(() => {
    ...
    toast({
      ...
      description: (() => {
        const skipped =
          (saveResult?.skipped_already_assigned ?? 0) + (saveResult?.skipped_not_found ?? 0)
        return skipped > 0
  ...
  const toggleDeviceName = React.useCallback((nhomId: number, name: string) => {
  ...
  const progressLabel =
    totalUniqueNames > 0
      ...
      : `${progress}%`
  const hasResults =
    status === "done" && ((result?.groups.length ?? 0) > 0 || (result?.unmatched.length ?? 0) > 0)
  const isEmptyResult =
    status === "done" &&
    result !== null &&
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] flex flex-col">
        ...
        {status === "error" && (
          <div className="space-y-3 rounded-md border border-destructive/50 bg-destructive/10 p-3 text-sm text-destructive">
            ...
            {canRetry && (
              <Button type="button" variant="outline" size="sm" onClick={retryFailedJob}>
                Thử lại
              </Button>
            )}
          </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-4">
            ...
            {result.groups.map((group) => (
              <SuggestedMappingGroupSection
                key={group.nhom_id}
                group={group}
                excludedDeviceNames={excludedDeviceNames.get(group.nhom_id) ?? new Set()}
                isGroupExcluded={excludedGroups.has(group.nhom_id)}
                onToggleDeviceName={(name) => toggleDeviceName(group.nhom_id, name)}
