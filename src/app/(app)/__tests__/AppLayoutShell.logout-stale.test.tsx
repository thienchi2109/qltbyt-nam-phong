...
const appLayoutShell = (
  <AppLayoutShell user={appLayoutUser}>
    <div>Child Content</div>
  </AppLayoutShell>
)

describe("AppLayoutShell logout stale screen guard", () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it("keeps the app shell hidden when session status returns authenticated before signout redirects", async () => {
    const user = userEvent.setup()
    let resolveUpdateSession: () => void = () => {}
    const updateSessionPromise = new Promise<void>((resolve) => {
      resolveUpdateSession = resolve
    })
    const signOutPromise = new Promise<void>(() => {})
    const sessionState = {
      data: { user: { id: "u1" } },
      status: "authenticated",
      update: mocks.updateSession,
    ...
    mocks.updateSession.mockReturnValue(updateSessionPromise)
    mocks.signOut.mockReturnValue(signOutPromise)

    const { rerender } = render(appLayoutShell)

    await user.click(screen.getByRole("button", { name: /đăng xuất/i }))

    sessionState.status = "loading"
    rerender(appLayoutShell)
