...
describe("troubleshootingDraft schema contract", () => {
  ...
  it("validates confidence level enum", () => {
    const invalid = {
      ...VALID_DRAFT,
      probable_causes: [{ label: "Test", confidence: "very-high", rationale: "test" }],
    }
    const result = troubleshootingDraftSchema.safeParse(invalid)
    expect(result.success).toBe(false)
  })
...
describe("troubleshooting draft evidence guard", () => {
  ...
  it("succeeds with valid evidence (equipmentLookup + operational source)", async () => {
    const result = await generateTroubleshootingDraft.execute!(VALID_TOOL_INPUT, {
      toolCallId: "t1",
      messages: [],
...
describe("troubleshooting draft safety", () => {
  it("draft tool is NOT in READ_ONLY_TOOL_DEFINITIONS", () => {
    expect(READ_ONLY_TOOL_DEFINITIONS_FOR_TEST).not.toHaveProperty("generateTroubleshootingDraft")
  })

  it("draft tool IS in DRAFT_TOOL_DEFINITIONS", () => {
    expect(DRAFT_TOOL_DEFINITIONS_FOR_TEST).toHaveProperty("generateTroubleshootingDraft")
  })
  ...
  it("system prompt version is v2.6.1", () => {
