...
export function buildSystemPrompt(context: SystemPromptContext = {}): string {
  ...
  return [
    ...
    [
      ...
      '- `modelSummary.itemCount`: dùng để trả lời số lượng (thay cho `total` trước đây).',
      '- `modelSummary.importantFields`: chứa dữ liệu quan trọng để suy luận (ví dụ: danh sách ứng viên danh mục, tên khoa/phòng).',
      '- `modelSummary.summaryText`: tóm tắt ngắn gọn kết quả.',
      '- Khi cần liệt kê chi tiết, tham chiếu dữ liệu từ `modelSummary.importantFields`.',
      '- KHÔNG tham chiếu `uiArtifact` trong câu trả lời — đó là dữ liệu dành cho giao diện.',
      '',
