...
import { QUERY_CATALOG, getQueryCatalogToolsByRoutingGroup } from "../tools/query-catalog"
...
describe("routeChatIntent", () => {
  ...
  describe("Issue #273 — curated precedence hardening", () => {
    ...
    it("preserves explicit repair-draft starts even when quota words are present", () => {
      ...
      expect(result).toEqual({
        kind: "proceed",
        requestedTools: FULL_PANEL_TOOLS.filter((toolName) => toolName !== "query_database"),
      })
    })
    ...
    ])('keeps clear curated prompt "$text" out of SQL fallback', ({ text, removedTools }) => {
      const result = routeText(text, [...ALL_CHAT_TOOLS])

      expect(result).toEqual({
        kind: "proceed",
        requestedTools: ALL_CHAT_TOOLS.filter((toolName) => !removedTools.includes(toolName)),
      })
    })

    it("keeps generic non-reporting prompts on the fail-closed tool-free path", () => {
  ...
  describe("Issue #272 — query_database curated-first fallback", () => {
    it("routes narrow reporting prompts to query_database when curated tools do not match", () => {
      const result = routeChatIntent({
        messages: [
          makeUserMessage("Báo cáo tổng hợp số lượng thiết bị theo trạng thái của đơn vị hiện tại"),
        ],
    ...
    it("strips query_database from repair-summary routing decisions", () => {
      ...
      expect(result).toEqual({
        ...
        requestedTools: ALL_CHAT_TOOLS.filter(
          (toolName) => toolName !== "equipmentLookup" && toolName !== "query_database"
        ),
      })
    })
    ...
    it("routes detailed reporting prompts with chi tiết to query_database when they are not specific-item lookups", () => {
      const result = routeChatIntent({
        messages: [
          makeUserMessage("Báo cáo chi tiết tình trạng thiết bị theo khoa trong đơn vị hiện tại"),
        ],
        requestedTools: [...ALL_CHAT_TOOLS],
      })
    ...
    it("holds all tools out of generic prompts when the request is not a reporting fallback", () => {
      const result = routeChatIntent({
