...
## Khảo sát live DB (read-only qua Supabase MCP)

Verified trên project `cdthersvldpnlbvpufrr` (2026-05-10):

| Verification | Kết quả |
|---|---|
| Migration `20260413152000_add_repair_cost_usage_visualizations` | Đã apply (registry version `20260414013614`) |
| `get_maintenance_report_data` security context | `SECURITY DEFINER`, `search_path=public, pg_temp`, owner postgres |
| GRANTs hiện tại | `anon`, `authenticated`, `postgres`, `service_role` |
...
### Phân tích live data (anonymized aggregate)

| Metric | Giá trị |
|---|---|
| Tổng yêu cầu hoàn thành (toàn DB) | 13 |
| Trung vị | ~18 giờ (1081 phút) |
| Trung bình | ~9,2 ngày |
| p90 | ~35 ngày |
| Min / Max | 0,5 phút / 39 ngày |
| Bucket counts | `<1d`: 7, `1-3d`: 2, `3-7d`: 1, `7-14d`: 0, `14-30d`: 0, `>30d`: 3 |
| Tỉ lệ đúng hạn (≤14 ngày) | 77% (10/13) |

**Bài học rút ra:**
- Phân bố **bimodal**: đa số xong nhanh (<3 ngày) + đuôi dài >30 ngày. Histogram là chart phù hợp để bộc lộ pattern này.
- Median <1 ngày ⇒ confirm cần **đơn vị tự động** (giờ/ngày).
- 3 ca <1h có thể là dữ liệu thật hoặc thao tác sai; plan KHÔNG filter — giữ trung thực, chỉ visualize.
- Quy mô data hiện tại nhỏ ⇒ rule "ignore tháng có completedCount < 3" trong monthly trend sẽ ẩn quá nhiều điểm. **Đổi rule**: render mọi tháng có ≥1 yêu cầu hoàn thành, tooltip hiển thị `completedCount` để user tự đánh giá độ tin cậy.
- Performance migration KHÔNG là vấn đề ở scale hiện tại; KHÔNG đề xuất thêm index mới.
...
Render mọi tháng có ≥1 yêu cầu hoàn thành (lý do: live data hiện chỉ 13 records — ẩn tháng ít data sẽ làm chart trống). Tooltip hiển thị `completedCount` để người xem tự đánh giá độ tin cậy.
