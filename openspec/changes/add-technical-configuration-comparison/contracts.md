...
- P10A1 owns `technical_configuration_comparison_get`, including database
  authorization, same-dossier guards, bounded criterion paging, ordered option
  aggregation, exact evidence summaries and the performance contract.
...
| P8A1  | `technical_configuration_suppliers_list`, `technical_configuration_supplier_create`, `technical_configuration_supplier_update`, `technical_configuration_supplier_delete`                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- Nullable comparison-set read request:
  `p_option_id`, `p_baseline_version_id`; response:
  `{ data: TechnicalConfigurationComparisonSetWire | null }`.
- `technical_configuration_comparison_set_get` authenticates and validates exact
  dossier ownership, remains readable after dossier archive and accepts no
  `p_expected_revision` because it performs no mutation. A missing pair returns
  `data: null` without creating a set, incrementing dossier revision or changing
  audit metadata.
- Comparison-matrix read request:
  non-null `p_baseline_version_id`, 1-8 non-null unique ordered
  `p_option_ids`, non-null `p_page >= 1` and non-null
  `1 <= p_page_size <= 100`.
- `technical_configuration_comparison_get` returns
  `{ data: { dossier, baseline_version, options, criteria }, total, page, page_size }`
  with the exact nested field/nullability contract in
  [p10-tdd-plan.md](./p10-tdd-plan.md#response-and-paging). Every evidence
  summary is exactly `{ document_count, citation_count, has_evidence }`; no
  citation/document array or reference-product aggregate is included.
- Comparison read errors are fixed: `42501/permission_denied` for missing
...
- The matrix payload is fetched through one bounded RPC. Supplier labels,
  responses, supplementary information and fixed-size evidence summaries
  (`document_count`, `citation_count`, `has_evidence`) must not trigger N+1
