...
Luồng save thủ công hiện chạy nhiều RPC group/criterion tuần tự nên không thể làm backend cho import nguyên tử. P5C thêm authoritative preview và atomic apply RPC cho toàn baseline. Hai RPC dùng chung một server-side validator/normalizer; apply revalidate dưới row lock, tăng revision một lần và rollback toàn bộ khi có lỗi. P5D chỉ gọi atomic apply sau confirmation, không chuyển preview thành chuỗi CRUD RPC.

File, canonical rows, preview và lỗi import là client state tạm thời, không được persist thành bảng hoặc record upload. UI giữ trạng thái này khi stale conflict và chỉ chặn lock affordance trong lúc unresolved import state còn hoạt động.
...
RPC chỉ trả evidence summary exact baseline/reference/option scope. Full
document/excerpt được lazy-load qua các bounded document RPC hiện có khi mở
