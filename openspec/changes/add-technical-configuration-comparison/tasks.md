...
| -------------------------------------------------------------------------------------------- | ---------------------------------------------- | --------------------- | ---------------------------------------- |
| [P0](./implementation-plan.md#phase-p0---discovery-and-contract-freeze)                      | Discovery và đóng băng contract                | Không                 | TC-01, TC-02, TC-19, TC-20               |
...
| [P5](./implementation-plan.md#phase-p5---baseline-excel-template-and-import)                 | Excel cấu hình cơ sở                           | P3B, P4               | TC-05, TC-20                             |
| [P6](./implementation-plan.md#phase-p6---shared-url-document-primitives)                     | Shared URL document primitives                 | P0; triển khai sau P5 | TC-11                                    |
| [P7A](./implementation-plan.md#phase-p7a---reference-products)                               | Sản phẩm tham chiếu                            | P3A, P4               | TC-02, TC-04, TC-06, TC-08, TC-20        |
| [P7B](./implementation-plan.md#phase-p7b---baseline-documents-and-citations)                 | Tài liệu và trích dẫn cấu hình cơ sở           | P4, P6                | TC-02, TC-04, TC-06, TC-11, TC-12, TC-20 |
...
| [P9A](./implementation-plan.md#phase-p9a---supplier-option-excel)                            | Excel phương án                                | P5, P8B               | TC-10, TC-20                             |
...
- [ ] P0.4 Chốt requirement traceability, authorization matrix và test matrix cho toàn roadmap.
- [ ] P0.5 Ghi feature baseline SHA dùng để audit toàn bộ rollout ở P13.
- [ ] P0.6 Review contract, cập nhật OpenSpec nếu cần và đóng issue không sửa production code.
...
- [ ] P2.1 Thêm schema/RPC cho nhóm và tiêu chí bản nháp.
- [ ] P2.2 Thêm ordering, criterion codes, multiline text và transactional mutations.
- [ ] P2.3 Thêm optimistic concurrency và structured errors.
- [ ] P2.4 Chạy DB phase gate cho quyền, grants/RLS, ownership/cascade và migration.
- [ ] P2.5 Viết tests cho hai cấp, reorder, duplicate code, rollback và conflict.
...
- [ ] P3B.1 Thêm editor hai cấp group/criterion.
- [ ] P3B.2 Thêm reorder, multiline content và explicit `Lưu`.
- [ ] P3B.3 Thêm unsaved, failed-save và optimistic-conflict preservation.
- [ ] P3B.4 Tích hợp editor vào workspace shell mà không làm shell phình to.
- [ ] P3B.5 Viết focused React tests và browser verification.

## Phase P3C - Bulk Text Entry

- [ ] P3C.1 Thêm bulk-entry dialog trong nhóm đang chọn.
- [ ] P3C.2 Parse text thành preview trước khi mutation.
- [ ] P3C.3 Không ghi dữ liệu khi preview còn lỗi.
- [ ] P3C.4 Giữ explicit-save contract sau khi bulk add.
- [ ] P3C.5 Viết parser, preview, cancel và persistence tests.

## Phase P4 - Baseline Versioning, Lock And History

- [ ] P4.1 Thêm state machine `Bản nháp`/`Đã khóa` và version history.
- [ ] P4.2 Enforce khóa bất biến ở database/backend, kể cả `admin/global`.
- [ ] P4.3 Thêm confirmation, hiển thị `locked_by`/`locked_at` và loại edit affordance.
- [ ] P4.4 Thêm expected-revision guard cho lock/copy và giữ form khi conflict.
- [ ] P4.5 Thêm tạo draft từ trống hoặc bản đã khóa và historical linkage.
- [ ] P4.6 Chạy DB phase gate và tests cho prerequisites/direct mutation/history.

## Phase P5 - Baseline Excel Template And Import

- [ ] P5.1 Thêm versioned system template bằng `exceljs`.
- [ ] P5.2 Thêm parser/validator và preview độc lập UI.
- [ ] P5.3 Thêm lỗi theo dòng, duplicate detection và atomic import.
- [ ] P5.4 Thêm expected-revision guard cho target draft và giữ preview khi conflict.
- [ ] P5.5 Thêm import UI chỉ cho bản nháp.
- [ ] P5.6 Viết round-trip, Unicode, multiline, malformed, conflict và version-mismatch tests.
...
- [ ] P7A.1 Thêm optional reference products thuộc baseline version.
- [ ] P7A.2 Thêm explicit save, dirty-state và expected-revision CRUD cho bản nháp.
- [ ] P7A.3 Enforce bất biến sau khóa và chạy DB phase gate.
- [ ] P7A.4 Thêm UI vào baseline workspace shell.
- [ ] P7A.5 Viết optional/multiple/conflict/reference-not-ranking tests.

## Phase P7B - Baseline Documents And Citations

- [ ] P7B.1 Thêm baseline document URL metadata và criterion citations.
- [ ] P7B.2 Reuse document cho nhiều tiêu chí không sao chép URL.
- [ ] P7B.3 Enforce locked immutability; editable delete phải xác nhận và nêu số liên kết.
- [ ] P7B.4 Thêm explicit save, dirty-state, expected-revision guard và tích hợp P6 primitives.
- [ ] P7B.5 Chạy DB phase gate cho quyền, ownership, cascade và concurrency.
- [ ] P7B.6 Viết ownership, reuse, deletion, locked, conflict và long-excerpt tests.
...
- [ ] P10B.1 Thêm sticky baseline/group và option columns.
- [ ] P10B.2 Thêm column selection, pinning, horizontal scroll và focus mode.
- [ ] P10B.3 Thêm detail panel cho full text, supplementary information và citations.
...
- [ ] P13B.3 Kiểm tra concurrent edits và conflict recovery qua hai tab.
- [ ] P13B.4 Chạy Equipment attachment regression và full relevant React tests.
- [ ] P13B.5 Chạy full React Doctor command và browser screenshot/interaction verification.
- [ ] P13B.6 Không sửa production code; mỗi gap tạo blocking fix leaf riêng rồi rerun P13B.
