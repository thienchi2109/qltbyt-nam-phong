...
- [ ] P8B1.1 Thêm UI nhóm nhẹ theo supplier, bật tab `options` và dùng nhãn
      `Nhà cung cấp · Model hoặc tên phương án`.
- [ ] P8B1.2 Thêm explicit create/update/delete cho supplier và option identity.
- [ ] P8B1.3 Xác nhận xóa option với response-dataset warning; xác nhận xóa supplier với affected-option count và cascade warning.
- [ ] P8B1.4 Hiển thị thời điểm cập nhật option metadata gần nhất.
- [ ] P8B1.5 Thêm conflict/reload, unsaved, pending, archived read-only và workspace-shell navigation states.
- [ ] P8B1.6 Viết multiple-option CRUD, delete, beforeunload, responsive và no-lock-control tests.
