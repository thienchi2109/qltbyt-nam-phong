...
- [ ] 1.1 Red: add contract tests for a read-only / RPC tool output envelope and for draft evidence surviving migration through `followUpContext`
- [ ] 1.2 Green: implement shared compaction helpers and envelope typing for migrated read-only / RPC tools only
- [ ] 1.3 Green: switch draft evidence collection to `followUpContext` for draft-eligible read-only / RPC tools without changing draft-tool output shape
- [ ] 1.4 Refactor: keep draft-tool outputs on their current raw path and isolate the carve-out/evidence parsing logic from generic compaction helpers
...
- [ ] 2.4 Refactor: align ranking/budget helpers and tool display metadata without widening pass-1 scope beyond the two migrated tools
