...
- [x] 2.1 Red: add tests for missing `device_name`, bounded `categorySuggestion` candidate retrieval, compact model-visible fields, and `departmentList` envelope resend behavior
- [x] 2.2 Green: update `categorySuggestion` prompt, tool schema, and SQL contract together so it asks first and returns top-k candidates
- [x] 2.3 Green: migrate `departmentList` to the shared read-only / RPC envelope contract with `uiArtifact = undefined`
- [x] 2.4 Green: update system prompt §4 to guide model reading envelope-shaped tool output (`modelSummary.itemCount` for counts, `uiArtifact.rawPayload.data` for lists) — required because Batch 1 changed the output shape from `{ data, total }` to `ToolResponseEnvelope`
