...
| 4.2  | Parent-category integration proves the direct count changes from `2` to `1` and the aggregate root total changes from `3` to `2`, without a double decrement.                                                                                                                      |
...
| 4.7  | An invalidated inactive unassigned-equipment query performs no immediate read, then refetches exactly once when a consumer mounts.                                                                                                                                                 |
| 4.8  | Focused matrix passed: 12 test files, 144 tests. It includes manual assignment, category aggregation, assigned equipment, role matrix, hardened RPC source contract, and RPC whitelist coverage.                                                                                   |
...
the unassignment hook, which requires a Query Client. The isolated tree suite now
mocks that hook, preserving its category-tree and aggregation boundary. The suite
passes 25 of 25 tests.
