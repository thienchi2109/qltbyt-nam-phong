...
- Usage scope: period mode uses valid completed usage intervals that start within the selected report range and do not finish after `dateRange.to`; cumulative mode uses valid completed usage intervals whose start and end are both on or before `dateRange.to`. A usage interval that crosses past the selected end date must be excluded rather than counted early.
