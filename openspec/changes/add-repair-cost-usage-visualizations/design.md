...
- Usage join: `nhat_ky_su_dung.thiet_bi_id` is `integer`, while `thiet_bi.id` and `yeu_cau_sua_chua.thiet_bi_id` are `bigint`; join through `thiet_bi.id` with an explicit migration comment noting the integer-to-bigint cast, and keep SQL smoke-test equipment IDs inside the integer range.
