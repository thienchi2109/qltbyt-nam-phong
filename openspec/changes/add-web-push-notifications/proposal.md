...
- Cấu hình recipient theo từng đơn vị bằng searchable multi-select account hiện hữu trong scope được server cấp; option hiển thị `full_name` và `username`, cho phép chọn/bỏ chọn bằng chuột hoặc bàn phím. UI không nhận username nhập tự do hoặc chuỗi phân cách bằng dấu phẩy; adapter giữ identity ổn định rồi gửi chuỗi username theo browser config contract. Lỗi bất kỳ tài khoản nào thì không lưu một phần.
...
- Người dùng đã đăng nhập tự bấm bật/tắt thông báo ngay trong app trên từng trình duyệt tại route authenticated `/notifications`; header bell giữ nguyên dialog hiện có và thêm đúng link `Cài đặt nhận thông báo` tới route này. Không thêm mục sidebar hoặc link trùng trong user menu; mục đổi mật khẩu/Đăng xuất giữ nguyên. Identity lấy từ NextAuth server session; không dùng `auth.users` UUID hoặc user ID do client tự khai. Mọi role đã đăng nhập đều thấy hướng dẫn/trạng thái browser opt-in; UI cấu hình recipient chỉ hiện cho `to_qltb/admin/global` theo phạm vi đơn vị.
...
- QLTBYT: session-authenticated candidate/configuration/subscription endpoints, UI quản trị và opt-in, service worker, private worker endpoints; tái sử dụng quyền và deep link hiện hành. Candidate lookup và config get/set đã được Phase 4.5 backend/RPC/API amendment enforce caller/target scope, protected self-membership, stale status và atomic preservation; Phase 5 chỉ tiêu thụ contract đã landed/live và không thêm backend/RPC/API. Không dùng account-list endpoint global-only làm nguồn chung cho mọi role. Maintenance catch-up của [#1001](https://github.com/thienchi2109/qltbyt-nam-phong/issues/1001) đã đóng với PASS theo evidence hiện hành; kết quả đó không thay thế aggregate Quality Gate.
