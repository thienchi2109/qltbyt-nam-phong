...
- [x] 7.1 Chứng minh create -> independent outboxes -> authenticated Oracle-style worker -> provider acceptance, đo target 60 giây khi khỏe và TTL <= remaining 24 giờ.
...
Checklist Phase 7/8 ở trên vẫn là hợp đồng acceptance lịch sử. Sau đó,
maintainer đã authorize riêng rollout live ngày 2026-09-19; authorization này
chỉ supersede boundary vận hành `production off`, không tự tick các task hoặc
chứng nhận formal Phase 7/8. Web Push cơ bản hiện operational dưới snapshot
live hiện tại; 7.1 nay được maintainer chấp thuận trên một sample live được
authorize, còn formal acceptance của Phase 7/8 vẫn partial.

| Item    | Status                                                           | Evidence và decision frontier                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------- | ---------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 7.1     | Checked - maintainer accepted; residual wire-TTL gap             | Request live mock `536` trên equipment `8392`: created `2026-09-19T08:00:33.476415Z`, materialized `08:00:36.341476Z`, terminal `08:00:37.504832Z`, accepted HTTP `201`, attempt `1`, failed `0`; terminal-minus-created `4.028417s` là upper bound vì provider network-accept timestamp không được lưu. Persisted deadline đúng `+86400s`. Maintainer acceptance đóng checkbox này, nhưng không relabel wire TTL, near-expiry/retry hoặc max-24h evidence chưa có thành `PASS`. |
| 7.2     | Partial                                                          | Session lifecycle có `10 files / 76 PASS` và parent `4 files / 27 PASS` trong [Phase 7B evidence](phase-7B-evidence.md). Fault matrix backend/Go/provider, partial delivery, credential failure, concurrent workers và expired backlog chưa được certify đầy đủ; DB gate `FAILED/INCOMPLETE` vẫn là accepted waiver, không phải `PASS`.                                                                                                                                          |
| 7.3     | Checked with explicit waiver                                     | Giữ nguyên wording maintainer đã chấp thuận và waiver không thu thập OS/browser matrix; không suy ra provider E2E hoặc mọi nền tảng đã được test.                                                                                                                                                                                                                                                                                                                                |
| 7.4     | Unknown / requires decision                                      | Artifact hiện tại không có evidence Phase 7 cho ZBS phone recipient, enqueue count và retry-state comparison. Cần planned local/disposable ZBS verification hoặc explicit waiver sau này; không suy luận từ test/runtime khác.                                                                                                                                                                                                                                                   |
| 7.5     | Partial                                                          | Enable/pause/rollback và read-back operational đã có trong [Phase 7A evidence](phase-7A-evidence.md), nhưng complete acceptance review, retention review và tất cả blockers chưa đóng.                                                                                                                                                                                                                                                                                           |
| 8.1-8.5 | Operational rollout evidence only; formal checklist remains open | Rollout 28 units/flags/worker readiness đã được authorize và ghi nhận; request `536` sau snapshot rollout cũng đã accepted, nhưng không tick Phase 8 vì các acceptance/matrix gaps trên còn mở.                                                                                                                                                                                                                                                                                  |
