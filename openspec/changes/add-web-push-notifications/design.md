...
UI cấu hình theo từng đơn vị chỉ hiển thị cho `admin/global` và `to_qltb` theo phạm vi caller. `admin/global` được target bất kỳ đơn vị nào nhưng candidate thường chỉ là account role `to_qltb` có effective unit `coalesce(current_don_vi, don_vi)` trùng target; `to_qltb` chỉ target effective unit của chính mình. Dùng searchable multi-select các account hiện hữu; option hiển thị `full_name` và `username`, hỗ trợ chọn/bỏ chọn bằng chuột hoặc bàn phím, cùng trạng thái loading/error/empty. UI giữ identity ổn định của option để tránh chọn nhầm, còn adapter serialize các username đã chọn theo browser config contract; server vẫn trim, loại trùng, resolve lại và lưu quan hệ user ID riêng từng recipient. `admin/global` có thể tự add/remove protected self-enrollment ở bất kỳ target; protected self-entry của caller khác chỉ read-only, còn stale/ineligible normal entry vẫn có thể được caller có quyền xóa khỏi cấu hình. Từ chối toàn bộ nếu unknown/inactive/không đủ quyền; bỏ selection hiện có là thao tác remove hợp lệ. Danh sách actual rỗng thì không push; protected self-enrollment còn lại vẫn là recipient và tiếp tục nhận. Không tự thêm global fallback.

Candidate lookup và semantics config protected đã được Phase 4.5 backend/API triển khai và landed/live; maintenance catch-up của [#1001](https://github.com/thienchi2109/qltbyt-nam-phong/issues/1001) đã đóng theo evidence hiện hành tại exact main commit `e9e2504ee30efed22c8b669cd94cbbb20c99cd37`. `web_push_recipient_config_get` trả recipient đã cấu hình; `user_list_for_admin` là danh sách global/admin không nhận `don_vi_id`, còn `don_vi_user_hierarchy` là global-only và chỉ dựng user theo `current_don_vi`. Không dùng hai nguồn đó trực tiếp cho picker của `to_qltb` hoặc làm bằng chứng đủ phạm vi. Phase 5 chỉ dùng adapter gọi contract server-authorized hiện có: nhận `don_vi_id` do picker chọn như target không tin cậy, trong khi server tự derive identity/role/effective unit từ session và authorize target trước khi trả candidate. Lookup có search và phân trang/limit bounded, chỉ trả field tối thiểu (`user_id`, `username`, `full_name`); config GET trả đầy đủ toàn bộ cấu hình hiện có để UI khởi tạo selection, gồm recipient stale/ineligible và protected self-entry. UI không được dựng lại selection từ các trang candidate; nếu chưa tải đủ config thì không cho Save. Stale/ineligible normal entry phải được flag và cho caller có quyền remove rõ ràng; protected self-entry của caller khác chỉ read-only, còn mọi protected entry được backend giữ lại trừ khi có self-action hợp lệ. Invalid protected entry không chặn normal save và không được đưa vào delivery. Candidate lookup không trộn stale/ineligible vào candidate mới. Save nhận self-action tường minh, từ chối atomic invalid additions/retentions, cho phép removals và preserve protected self entries. Adapter này không cho browser đọc bảng `nhan_vien` trực tiếp. Phase 5 không thêm hoặc sửa API/RPC/backend/SQL.

### Phase 5 UI allocation

Phase 5 được phân thành đúng năm chunk review, tất cả checkbox giữ unchecked cho tới khi có evidence:

1. **5.1 Recipient picker:** searchable multi-select account có sẵn theo từng `don_vi`, không nhập username tự do hoặc CSV; vị trí hiển thị đã chốt tại route authenticated `/notifications`, dùng chung cho mọi authenticated role, nhưng phần recipient config chỉ hiện cho `to_qltb/admin/global` theo phạm vi đơn vị.
2. **5.2 Configuration status:** trạng thái cấu hình, protected self-entry, stale/ineligible entry và quyền sửa/xem/gỡ tương ứng; giữ selection stale qua search/reload.
3. **5.3 Subscription opt-in:** user gesture, permission prompt, enabled/disabled/error/unsupported states, accessible feedback và registration qua worker hiện có.
4. **5.4 Notification operation status:** trạng thái local của thao tác, retry/cancel/error, revoke/logout/account switch và refresh/reload; không suy diễn delivery/provider delivered/read.
5. **5.5 Integration and polish:** tích hợp trang, worker push/click, responsive/accessibility, focused user-event tests và polish cuối.

Placement Phase 5 đã được chốt là route authenticated `/notifications` dùng chung cho mọi role đã đăng nhập. Header bell giữ nguyên `NotificationBellDialog` hiện có và thêm link có text chính xác `Cài đặt nhận thông báo` tới `/notifications`; giữ nguyên đổi mật khẩu/Đăng xuất, không thêm mục sidebar và không thêm link trùng trong user menu. Mọi người dùng đã đăng nhập thấy hướng dẫn và trạng thái browser opt-in; chỉ `to_qltb/admin/global` thấy recipient config, với `admin/global` có target selector cross-unit còn `to_qltb` chỉ thấy effective unit của mình. Bằng chứng UI hiện có: route `repair-requests` cũng là authenticated và dùng được cho `to_qltb`, còn `AppLayoutShell` có header `NotificationBellDialog` và user menu dùng chung. `/users` và `/tenants` không phải surface chung vì route hiện có policy global-only; không dùng chúng làm entrypoint. Năm chunk chỉ bao phủ frontend/browser lifecycle và tests, không bao phủ backend/API/RPC/SQL.
...
Phase 1 truth table và Phase 1 evidence vẫn là historical record, không sửa lại. Phase 4.5 đã thay thế predicate eligibility của recipient trong configuration, enqueue và claim/retry: recipient thường phải là `to_qltb` có effective unit trùng target; target authorization khác nhau giữa `admin/global` và `to_qltb`; protected self-enrollment là membership do caller sở hữu và phải giữ provenance riêng. Quyền đọc request và các guard đọc request hiện hành vẫn tách biệt, giữ nguyên và không được mở rộng bởi configuration. Phase 5 chỉ hiển thị và điều khiển browser state qua contract đã có.
