...
UI cấu hình theo đơn vị của thiết bị, cho global/admin và to_qltb đúng scope. Dùng searchable multi-select các account hiện hữu; option hiển thị `full_name` và `username`, hỗ trợ chọn/bỏ chọn bằng chuột hoặc bàn phím, cùng trạng thái loading/error/empty. UI giữ identity ổn định của option để tránh chọn nhầm, còn adapter serialize các username đã chọn theo contract v1; server vẫn trim, loại trùng, resolve lại và lưu quan hệ user ID riêng từng recipient. Reject toàn bộ nếu unknown/inactive/không đủ quyền. Danh sách rỗng là tắt recipient Web Push của đơn vị. Không tự thêm global fallback.

Candidate lookup là dependency backend/API của Phase 5. `web_push_recipient_config_get` chỉ trả recipient đã cấu hình; `user_list_for_admin` là danh sách global/admin không nhận `don_vi_id`, còn `don_vi_user_hierarchy` là global-only và chỉ dựng user theo `current_don_vi`. Không dùng hai nguồn đó trực tiếp cho picker của `to_qltb` hoặc làm bằng chứng đủ scope. Runtime cần một server-authenticated adapter/RPC nhận `don_vi_id` được picker chọn như target không tin cậy; server phải tự derive identity/role/scope từ session và authorize target trước khi trả account đủ điều kiện. Lookup cần search và pagination/limit bounded, chỉ trả minimal fields (`user_id`, `username`, `full_name`); adapter này không đổi wire PUT v1 và không được cho browser đọc bảng `nhan_vien` trực tiếp. Nếu backend dependency chưa có, Phase 5 dừng ở review boundary trước khi viết UI runtime; mọi thay đổi API/RPC/backend phải được báo cáo và phê duyệt riêng trước implementation.
