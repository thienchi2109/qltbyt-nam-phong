...
Phase 1 truth table và Phase 1 evidence vẫn là historical record, không sửa lại. Planned Phase 4.5 thay thế predicate eligibility của recipient trong configuration, enqueue và claim/retry: recipient thường phải là `to_qltb` có effective unit trùng target; target authorization khác nhau giữa `admin/global` và `to_qltb`; protected self-enrollment là membership do caller sở hữu và phải giữ provenance riêng. Quyền đọc request và các guard đọc request hiện hành vẫn tách biệt, giữ nguyên và không được mở rộng bởi configuration cho đến khi amendment được triển khai và verify.
