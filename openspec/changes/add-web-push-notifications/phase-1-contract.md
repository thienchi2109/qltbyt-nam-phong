...
| ---------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `GET /api/web-push/config?don_vi_id=<decimal>` | Historical v1 `{version:1, don_vi_id, recipients:[{user_id, username}]}`; planned Phase 4.5 amendment adds status/protection fields and caller-scope enforcement |
| `PUT /api/web-push/config`                     | Historical v1 `{version:1, don_vi_id, usernames:"a, b"}`; planned Phase 4.5 amendment adds explicit protected-self action and amended response                   |
...
### Phase 4.5 - Bổ sung config cho browser (chưa triển khai)

Các dòng config API ở trên là baseline browser v1 mang tính lịch sử. Phase 4.5 phải bổ sung contract tối thiểu cho browser để biểu diễn protected self-membership và trạng thái stale; đây là amendment được hoạch định, chưa phải runtime hiện hành. Worker claim/report wire v1 giữ nguyên.

- `GET /api/web-push/candidates?don_vi_id=<decimal>&q=<text>&limit=<1..100>&cursor=<opaque>` → `{version:1, don_vi_id, candidates:[{user_id, username, full_name}], next_cursor:<opaque|null>}`. Candidate chỉ gồm recipient thường còn đủ điều kiện: role `to_qltb` có effective unit `coalesce(current_don_vi, don_vi)` bằng target. Server tự derive identity/role/effective unit của caller và authorize target; `don_vi_id`, query, limit và cursor đều là input không tin cậy. `admin/global` được target bất kỳ đơn vị nào; `to_qltb` chỉ được target effective unit của chính mình.
- Response config GET được hoạch định phải giữ toàn bộ entry đang cấu hình, gồm stale/ineligible và protected self-entry, kèm metadata trạng thái tối thiểu như `{user_id, username, full_name, status, protected, editable}`. Đây là nguồn đầy đủ để UI khởi tạo selection; UI không được dựng lại selection từ kết quả candidate có phân trang và không được Save khi config chưa tải đủ. Stale/ineligible normal entry phải được flag để caller có quyền remove tường minh; protected self-entry của caller khác chỉ read-only. Backend phải giữ toàn bộ protected entry, gồm entry của chính caller, khi `self_action=none` hoặc không có self-action hợp lệ; invalid protected entry không chặn normal save và không được enqueue/claim. Backend cũng giữ stale/ineligible normal entry trong DB cho tới khi có valid save hoặc explicit removal; các entry stale/invalid không được âm thầm chuyển đơn vị hoặc xóa.
- Planned config PUT vẫn dùng `usernames` làm serialization recipient thường và thêm `self_action: "none"|"add"|"remove"` tường minh cho protected self-enrollment của caller. `admin/global` được self-add/remove ở bất kỳ target; caller khác không được sửa protected self-entry của caller khác. Server từ chối atomic nếu request thêm hoặc giữ lại normal recipient invalid; bỏ entry khỏi selection đầy đủ là thao tác remove hợp lệ. Save là tường minh, không auto-save.
- Cấu hình actual rỗng nghĩa là không enqueue Web Push cho đơn vị đó. Protected self-enrollment còn lại vẫn là recipient và tiếp tục nhận. Quyền đọc request và các subject check của enqueue/claim vẫn tách biệt; configuration không cấp quyền đọc request.

### Làm rõ candidate picker Phase 5 (ranh giới wire)

Phase 5 sử dụng amendment config browser được hoạch định ở Phase 4.5. UI có thể hiển thị option `{user_id, username, full_name}` và giữ identity đã chọn, nhưng mutation bình thường dùng serialization `usernames` và `self_action` tường minh; không tạo worker wire riêng. Picker có thể gửi `don_vi_id` đã chọn như target không tin cậy; server tự derive identity/role/effective unit từ session và authorize target, không nhận role/tenant/identity do client khai báo. Candidate lookup dùng search/phân trang có giới hạn và field tối thiểu; browser không bao giờ đọc trực tiếp `nhan_vien`.

`user_list_for_admin()` hiện là global/admin-only, không nhận đơn vị; `don_vi_user_hierarchy(p_q, p_only_active)` là global-only và trả user theo `current_don_vi`. Hai nguồn hiện hữu không phải candidate API dùng chung cho Phase 5, đặc biệt không đủ cho `to_qltb`. Adapter/API/RPC cụ thể, metadata protected-entry và forward-only migration là dependency implementation của Phase 4.5, chưa được triển khai bởi phần làm rõ contract này. Thay đổi backend/API/RPC cần báo cáo và phê duyệt riêng trước implementation. Không sửa migration đã applied hoặc historical evidence; worker wire v1 giữ nguyên.
