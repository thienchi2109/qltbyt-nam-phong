...
- [ ] 1.2 Persist the regulatory document, catalog version, sections, items,
      rules, source pages, and source references without exposing unit write access.
- [ ] 1.3 Add source-traceability tests for all five sections and 37 equipment
      items, including multiline rules and footnote references.

## 2. Draft Persistence Contract

- [ ] 2.1 Add the unit draft and draft-item data model with one-editable-draft
      per unit and duplicate-item protection.
- [ ] 2.2 Add RPCs for create-or-open, read, save, exclude, and restore draft
      operations with session-unit and role guards.
- [ ] 2.3 Add audit records for draft creation and unit-specific mutations.
- [ ] 2.4 Add the RPC proxy allowlist entries and generated database contract
      updates required by the new RPCs.

## 3. Draft Editor UX

- [ ] 3.1 Replace the current single-category create action with a draft
      editor entry point in `/device-quota/categories`.
- [ ] 3.2 Render all regulatory rows grouped by the five read-only sections and
      visually distinguish regulatory fields from unit-specific fields.
- [ ] 3.3 Implement editable display name, applied unit, applied quantity,
      notes, exclude, restore, save, reopen, and view states.
- [ ] 3.4 Resolve the unit from the authenticated session and prevent a
      user-controlled unit override in this flow.
- [ ] 3.5 Preserve existing category, assignment, active-decision, compliance,
      and report surfaces while a draft is being edited.

## 4. Validation And Regression Coverage

- [ ] 4.1 Add domain and RPC tests for tenant isolation, duplicate rows,
      missing-session-unit failure, non-negative integer quantities, and
...
- [ ] 4.3 Add regression tests proving draft writes do not invalidate or
      mutate active decision, equipment-mapping, compliance, or report data.
