...
### Phase 5: Markdown table presentation

QLTBYT prompt mặc định hướng dẫn dùng bảng Markdown cho nhiều mục cùng thuộc tính hoặc so sánh; ưu tiên 3–5 cột, ô ngắn, dữ liệu thiếu ghi “Chưa có dữ liệu”, không suy diễn để điền ô. Giải thích dài, hướng dẫn thao tác và clarification vẫn dùng đoạn văn/danh sách; tool/artifact cards giữ nguyên.

Tái sử dụng renderer `react-markdown` + `remark-gfm`. Phase 5 kiểm tra dark BFF giữ text qua chunk boundaries, bảng hoàn tất đủ dòng/cột và cuộn ngang trên mobile không tràn trang. Fixtures bao gồm Unicode, pipe được escape và nội dung ô không tin cậy. Prompt contract/UI fixtures không bảo đảm mọi output provider tuân thủ định dạng; gate không cần paid provider smoke. Chưa triển khai hành vi này trong Phase 3.
