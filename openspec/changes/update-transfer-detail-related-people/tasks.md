...
- [ ] 1.1 Add a failing interaction test proving opening the Transfers detail dialog from a list row with requester and approver IDs only calls `transfer_request_get(p_id)` and renders requester and approver after resolution
...
- [ ] 1.3 Add a failing test proving unresolved requester or approver user references do not crash the dialog and only render resolvable related-person rows
...
- [ ] 2.1 Add `transfer_request_get(p_id int)` so the transfer detail payload includes nullable nested `nguoi_yeu_cau` and `nguoi_duyet` objects when resolvable
- [ ] 2.2 Whitelist any new transfer detail RPC in the RPC proxy if required
- [ ] 2.3 Update the Transfers detail-open flow so opening from a list row resolves `transfer_request_get(p_id)` before rendering `Người liên quan`
- [ ] 2.4 Render `Người yêu cầu` and `Người duyệt` from the resolved detail payload only when each user object is present, while keeping non-people sections unchanged
...
- [ ] 3.1 Update frontend transfer types so the `transfer_request_get(p_id)` detail payload models nested requester and approver users explicitly, separately from list-row DTOs where needed
